...
class Settings(BaseSettings):
    ...
    DB_URL: SecretStr = Field(
        default="sqlite+aiosqlite:///:memory:",  # type: ignore[arg-type]
        description="SQLAlchemy database URL, e.g. postgresql+asyncpg://user:pass@host/db",
    )
    JWT_SECRET: SecretStr = Field(
        default="dev-only-not-secret-change-in-prod-1234567890",  # type: ignore[arg-type]
        min_length=32,
        description="HS256 secret, at least 32 chars",
    )
    ...
    EVENT_BROKER_URL: str = Field(default="localhost:9092", description="Kafka broker URL")
